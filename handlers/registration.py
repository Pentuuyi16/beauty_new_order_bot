...
async def process_customer_gdpr_accept(callback: CallbackQuery, state: FSMContext, db: Database, bot: Bot):
    ...
    if not trial_used and not has_subscription:
        text = (
            ...
            "Важно ❗️\n"
            "Перед переходом в чат необходимо оформить подписку.\n\n"
            "🔒 Без активной подписки размещение объявлений невозможно.\n\n"
            "📌 Объявления (заявки на процедуры) формируются только в этом боте, через специальную форму.\n\n"
            "❗️Чат предназначен только для публикации уже созданных объявлений.\n"
            "Сообщения, написанные вручную в чате, не учитываются и не обрабатываются.\n\n"
            "Что нужно сделать дальше:\n"
            "1️⃣ Оформить подписку\n"
            "2️⃣ После оплаты откроется доступ к созданию объявления\n"
            "3️⃣ Бот автоматически опубликует ваше объявление в чате\n\n"
            "🎁 СПЕЦИАЛЬНОЕ ПРЕДЛОЖЕНИЕ!\n"
            "Получите БЕСПЛАТНЫЙ месяц подписки прямо сейчас!"
        )
    else:
        text = (
            "✅ Регистрация успешно завершена\n\n"
...
async def process_model_gdpr_accept(callback: CallbackQuery, state: FSMContext, db: Database, bot: Bot):
    ...
    # ДОБАВЛЯЕМ КНОПКУ "Перейти к заявкам"
